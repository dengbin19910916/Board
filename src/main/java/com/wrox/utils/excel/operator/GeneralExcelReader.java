...
public class GeneralExcelReader implements ExcelReader {
    ...
    public GeneralExcelReader(Workbook workbook) {
    ...
    private String getCellValue(@NotNull Cell cell, @NotNull ExcelError error) {
        ...
        switch (cellType) {
            case Cell.CELL_TYPE_BLANK:      // 单元格数据为空时直接返回null，表示此单元格数据无效。
                log.warn("单元格{}为空。", new org.apache.poi.ss.util.CellReference(cell).formatAsString());
                return "";
            case Cell.CELL_TYPE_ERROR:      // 单元格数据错误，返回错误信息。
                error.put(new org.apache.poi.ss.util.CellReference(cell), "单元格数据错误[" + FormulaError.forInt(cell.getErrorCellValue()).getString() + "]。");
            ...
            default:
                error.put(new org.apache.poi.ss.util.CellReference(cell), "未知的单元格数值[" + formatter.formatCellValue(cell) + "]！");
